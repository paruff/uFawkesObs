...
class TestPrometheusGlobalConfig:
    ...
    def test_scrape_interval_valid_format(self, prometheus_config_path):
        ...
        assert isinstance(scrape_interval, str), "scrape_interval should be a string"
    ...
    def test_external_labels_format(self, prometheus_config_path):
        ...
        if "external_labels" in config["global"]:
            ...
            assert isinstance(labels, dict), "external_labels should be a dictionary"
...
class TestPrometheusSelfMonitoringRules:
    ...
    def test_self_monitoring_rule_file_exists(self, project_root):
        ...
        rule_file = (
            ...
            / "prometheus"
            / "rules"
            / "ufawkesobs-self-monitoring.yml"
        )
    ...
    def test_self_monitoring_rules_have_required_labels(self, project_root):
        ...
        assert len(rules) > 0, (
            "Expected at least one alert rule in self-monitoring file"
        )
        for rule in rules:
            ...
            assert "severity" in labels, (
                f"Alert {rule['alert']} should include severity label"
            )
            assert "category" in labels, (
                f"Alert {rule['alert']} should include category label"
            )
...
class TestPrometheusScrapeConfigs:
    ...
    def test_scrape_configs_exists(self, prometheus_config_path):
        ...
        assert "scrape_configs" in config, "scrape_configs section should be present"
        assert isinstance(config["scrape_configs"], list), (
            "scrape_configs should be a list"
        )
        assert len(config["scrape_configs"]) > 0, "scrape_configs should not be empty"
...
class TestPrometheusJobsForObservabilityStack:
    ...
    def test_otel_collector_job_exists(self, prometheus_config_path):
        ...
        assert len(otel_jobs) > 0, "OTel Collector scrape job should be configured"
