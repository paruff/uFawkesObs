...
- **Second OTel Collector:** `otel-collector-dora` compose service, gated behind the `dora` profile, configured via `config/otel/collector-dora.yaml`. Filters `dora.*`/`cicd.*`/`deployment.*`/`incident.*` OTLP metrics and exposes them via its own Prometheus exporter — it does not forward to `dora-api`, which is a REST-only ingestion endpoint (`POST /event`), not an OTLP receiver.
